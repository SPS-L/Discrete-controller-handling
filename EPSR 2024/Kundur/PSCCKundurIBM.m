...
while aaa<5
    aaa=aaa+1;
clearvars -except aaa tsimave SRMt SRMx SRMh ATMt ATMx SSMt SSMx SSMh
clc
tic
...
legend('V_{1} SRM','V_{1} ATM','V_{1} SSM','V_{1} IBM-A','Location','southeast')
set(gca,'FontSize',18)
xlim([0 tsim])

axes('position',[.3 .22 .38 .38])
box on % put box around new pair of axes
signalIBM=sqrt((xhist(49,:)).^2+(xhist(50,:)).^2);
signalSSM=sqrt((SSMx(49,:)).^2+(SSMx(50,:)).^2);
signalATM=sqrt((ATMx(73,:)).^2+(ATMx(74,:)).^2);
signalSRM=sqrt((SRMx(49,:)).^2+(SRMx(50,:)).^2);
indexOfInterestSRM = (SRMt > 1.5) & (SRMt < 3); % range of t near perturbation
indexOfInterestATM = (ATMt > 1.5) & (ATMt < 3); % range of t near perturbation
indexOfInterestSSM = (SSMt > 1.5) & (SSMt < 3); % range of t near perturbation
indexOfInterestIBM = (times > 1.5) & (times < 3); % range of t near perturbation
plot(SRMt(indexOfInterestSRM),signalSRM(indexOfInterestSRM)) % plot on new axes
hold on
plot(ATMt(indexOfInterestATM),signalATM(indexOfInterestATM),':') % plot on new axes
plot(SSMt(indexOfInterestSSM),signalSSM(indexOfInterestSSM),'g--') % plot on new axes
plot(times(indexOfInterestIBM),signalIBM(indexOfInterestIBM),'+') % plot on new axes
...
if save==1
    hgexport(gcf,'PSCCKundurVoltage.pdf',figure_property);
end
...
% plot(times,ehist(4,:),'+')
xlabel('Time (s)','FontSize', 24)
ylabel('Governor signal (Per unit)','FontSize', 24)
legend('G3 SRM','G3 ATM','G3 SSM','G3 IBM-A','Location','southeast')
set(gca,'FontSize',18)
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'PSCCKundurGovernor.pdf',figure_property);
end
figure (32) %Exciter
% plot(times,ehist(5,:),'+')
% hold on
% plot(times,ehist(6,:),'+')
plot(times,ehist(7,:).*2032.12403100775,'+')
% plot(times,ehist(8,:),'+')
box on
hold on
xlabel('Time (s)','FontSize', 24)
ylabel('Exciter signal (Per unit)','FontSize', 24)
legend('G3 SRM','G3 ATM','G3 SSM','G3 IBM-A','Location','northeast')
set(gca,'FontSize',18)
xlim([0 tsim])
ylim([0.9 2.1])

axes('position',[.35 .5 .38 .38])
box on % put box around new pair of axes
ExsignalIBM=ehist(7,:).*2032.12403100775;
ExsignalSSM=SSMh(7,:).*2032.12403100775;
ExsignalATM=ATMx(31,:).*2032.12403100775;
ExsignalSRM=SRMh(7,:).*2032.12403100775;
indexOfInterestSRM = (SRMt > 1) & (SRMt < 1.6); % range of t near perturbation
indexOfInterestATM = (ATMt > 1) & (ATMt < 1.6); % range of t near perturbation
indexOfInterestSSM = (SSMt > 1) & (SSMt < 1.6); % range of t near perturbation
indexOfInterestIBM = (times > 1) & (times < 1.6); % range of t near perturbation
plot(SRMt(indexOfInterestSRM),ExsignalSRM(indexOfInterestSRM)) % plot on new axes
hold on
plot(ATMt(indexOfInterestATM),ExsignalATM(indexOfInterestATM),':') % plot on new axes
plot(SSMt(indexOfInterestSSM),ExsignalSSM(indexOfInterestSSM),'g--') % plot on new axes
plot(times(indexOfInterestIBM),ExsignalIBM(indexOfInterestIBM),'+') % plot on new axes
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'PSCCKundurExciter.pdf',figure_property);
end
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'PSCCKundurSpeedDeviation.pdf',figure_property);
end
...
legend('SRM','ATM','SSM','IBM-A','Location','northwest')
set(gca,'FontSize',18)
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'PSCCKundurStepSize.pdf',figure_property);
end
...
while j<a-1
    j=j+1;
        ...
        accsd3=(IBMSD3(1,j)-SRMSD3(1,j))^2;
...
%                    aw=min(aw,0.01); %lower limnit anti wind up
                    aw=min(aw,0.01); %lower limnit anti wind up
                if cni==7
                    aw=min(aw,2/2032.12403100775); %lower limnit anti wind up
                end
                    f2=eegm(mm,cni)-aw;
            else
                if cni==5
...
%        egtemp=min(egtemp,0.01); %lower limnit anti wind up
        egtemp=min(egtemp,2/2032.12403100775); %lower limnit anti wind up
    end
