...
while aaa<1
    aaa=aaa+1;
clearvars -except aaa tsimave SRMt SRMx SRMh ATMt ATMx
clc
    ...
    if t<min(T)
        ...
        if sum(mm)==0
            ...
            while jjj<cn
                jjj=jjj+1;
                if mm(1,jjj)~=0
                    [et,avrov]=contex(mm,et,T,cn,avrov,xx,jjj); %Calculating the controllers' signal in case of any event
                end
            end
            et=quz(et,cq);
            flagg=1;
...
figure (1) %voltage
plot(times,sqrt((xhist(49,:)).^2+(xhist(50,:)).^2),'g--')
hold on
...
% plot(times(indexOfInterest),signal(indexOfInterest),'g--') % plot on new axes
% hold on
...
% hold on
% plot(times,ethist(6,:))
stairs(times,ethist(7,:).*2032.12403100775,'g--')
% plot(times,ethist(8,:))
hold on
xlabel('Time','FontSize', 24)
...
SSMV1=sqrt(xhist(49,:).^2+xhist(50,:).^2);
%Speed deviation 3
SRMSD3=vq(11,:);
SSMSD3=xhist(11,:);
...
while j<a
    j=j+1;
        accv1=(SSMV1(1,j)-SRMV1(1,j))^2;
        accv1sum=accv1sum+accv1;
        accsd3=(SSMSD3(1,j)-SRMSD3(1,j))^2;
...
function [etj,avrov]=contex(mm,etj,T,cn,avrov,xx,jjj)
contj=jjj;
