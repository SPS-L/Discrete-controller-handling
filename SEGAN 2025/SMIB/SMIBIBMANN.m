...
T=[0.02 0.006]; %Time of sampling by the controllers
Tm=0.5013;
...
while t<tsim
    ...
    [iterations,xx,eg,tg,feeg,eeg,avrov,gvrov]=NewtonNL(h,holdd,xx0,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,tpre2,t,sg,eg,T,tg,ee0gm,ee0gv,egpre,cn,cq,dae,avrovpre,avrovmm,gvrovpre,gvrovmm); %The main function (Newton solver)
...
figure(1)
plot(times,sqrt(xhist(14,:).^2+xhist(15,:).^2),'r+')
hold on
% plot(times,xhist(10,:),'+')
xlabel('Time (s)','FontSize', 24)
ylabel('Voltage (Per unit)','FontSize', 24)
set(gca,'FontSize',18)
legend('V_{g} (SRM)','V_{g} (SSM)','V_{g} (IBM)','Location','southeast')
...
figure(2)
plot(times,sqrt((xhist(18,:).^2)+(xhist(19,:).^2)),'r+')
% plot(times,xhist(13,:),'+')
hold on
% plot(times,xhist(14,:),'+')
...
set(gca,'FontSize',18)
legend('I_{g} (SRM)','I_{g} (SSM)','I_{g} (IBM)','Location','northeast')
set(gca, 'FontName', 'Times New Roman')
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
set(gca,'FontSize',18)
legend('SRM','SSM','IBM','Location','southeast')
set(gca, 'FontName', 'Times New Roman')
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
figure(7)
plot(times,hhist(5,:).*(1/0.000747681),'r+')
hold on
xlabel('Time (s)','FontSize', 24)
...
figure (8)
plot(times,sqrt((xhist(14,:).^2)+(xhist(15,:).^2)),'--*')
hold on
xlabel('Time (s)','FontSize', 24)
...
set(gca,'FontSize',18)
legend('SRM','SSM','IBM','Location','northwest')
...
figure(20)
plot(times,xhist(14,:).*xhist(18,:)+xhist(15,:).*xhist(19,:),'r+')
hold on
xlabel('Time','FontSize', 24)
...
hold on
figure(21)
plot(times,xhist(15,:).*xhist(18,:)-xhist(14,:).*xhist(19,:),'r+')
hold on
xlabel('Time','FontSize', 24)
...
plot(times,xhist(6,:),'*')
hold on
plot(times,xhist(20,:).*xhist(17,:)-xhist(21,:).*xhist(16,:),'*')
xlabel('Time','FontSize', 24)
...
function [iterations,xx,eg,tg,feeg,eegv,avrov,gvrov]=NewtonNL(h,holdd,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,tpre2,t,sg,eg,T,tg,eegm,eegv,egpre,cn,cq,dae,avrovpre,avrovmm,gvrovpre,gvrovmm)
...
function [f2,gvrov]=contim(eegm,egpre,T,xg,m,mm,cni,avrovpre,avrovmm,gvrovpre,gvrovmm)
if cni==1
            if m==1
                [aw,gvrov]=torque(egpre(1,cni),T(1,cni),xg,gvrovpre);
                aw=max(aw,0.1); %upper limnit anti wind up
                aw=min(aw,0.9); %lower limnit anti wind up
                f2=eegm(mm,cni)-aw;
                avrov=0;
            else
                [aw,gvrov]=torque(eegm(mm-1,cni),T(1,cni),xg,gvrovmm(1,m-1));
                aw=max(aw,0.1); %upper limnit anti wind up
                aw=min(aw,0.9); %lower limnit anti wind up
                f2=eegm(mm,cni)-aw;
                avrov=0;
...
elseif cni==2
            Vter=sqrt(xg(14,1)^2+xg(15,1)^2);
            global net;
            global fuzzyAVR;
            if m==1
%                 [aw]=sim(net,Vter); %Controller's equation
%                 aw=net(Vter);
                aw=evalfis(fuzzyAVR,Vter);
...
if cni==1
    ...
    egtemp=max(egtemp,0.1); %upper limnit anti wind up
    egtemp=min(egtemp,0.9); %lower limnit anti wind up
elseif cni==2
    global net;
    global fuzzyAVR;
    Vter=sqrt(xg(14,1)^2+xg(15,1)^2);
%     [egtemp]=sim(net,Vter); %Controller's equation
%     egtemp=net(Vter);
    egtemp=evalfis(fuzzyAVR,Vter);
