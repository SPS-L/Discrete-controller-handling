...
while t<tsim
    ...
    if t<min(T)
        ...
        if mm==0
            ...
            etpre2=etpre;
            etpre=et;
            [et,avrov,gvrov]=contex(conti,et,T,cn,avrov,gvrov,xx,net); %Calculating the controllers' signal in case of any event
            et=quz(et,cq);
...
figure(2)
% plot(times,xhist(13,:))
plot(times,sqrt((xhist(18,:).^2)+(xhist(19,:).^2)))
hold on
% plot(times,xhist(14,:))
xlabel('Time','FontSize', 24)
...
figure(7)
plot(times,hhist(5,:).*(1/0.000747681))
hold on
xlabel('Time','FontSize', 24)
...
figure(20)
plot(times,xhist(14,:).*xhist(18,:)+xhist(15,:).*xhist(19,:))
xlabel('Time','FontSize', 24)
ylabel('Active power (Per unit)','FontSize', 24)
set(gca,'FontSize',18)
hold on
figure(21)
plot(times,xhist(15,:).*xhist(18,:)-xhist(14,:).*xhist(19,:))
xlabel('Time','FontSize', 24)
...
function [etj,avrov,gvrov]=contex(conti,etj,T,cn,avrov,gvrov,xx,net)
global fuzzyAVR
jj=size(conti);
jj=jj(1,2);
j=0;
