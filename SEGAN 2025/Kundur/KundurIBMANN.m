...
tsim=25; %Time of simulation
%Initial state variables values
[ssi]=ini;
xx=ssi;
dae=[28 79];
cn=9; %Number of controllers
T=[0.02 0.03 0.04 0.05 0.005 0.006 0.007 0.008 0.09]; %Time of sampling by the controllers
...
% eg=[0.952041625976563,0.545169830322266,0.435935974121094,0.226100921630859,0.000862121582031250,0.000545501708984375,0.000553131103515625,0.000377655029296875,0.107257843017578];
% egpre=[0.952041625976563,0.545169830322266,0.435935974121094,0.226100921630859,0.000862121582031250,0.000545501708984375,0.000553131103515625,0.000377655029296875,0.107257843017578];
% egpre2=[0.952041625976563,0.545169830322266,0.435935974121094,0.226100921630859,0.000862121582031250,0.000545501708984375,0.000553131103515625,0.000377655029296875,0.107257843017578];

% eg=[0.954813241958618,0.546663284301758,0.433381319046021,0.224491834640503,0.000861883163452148,0.000544548034667969,0.000554561614990234,0.000378847122192383,0.113649845123291];
% egpre=[0.954813241958618,0.546663284301758,0.433381319046021,0.224491834640503,0.000861883163452148,0.000544548034667969,0.000554561614990234,0.000378847122192383,0.113649845123291];
% egpre2=[0.954813241958618,0.546663284301758,0.433381319046021,0.224491834640503,0.000861883163452148,0.000544548034667969,0.000554561614990234,0.000378847122192383,0.113649845123291];

eg=[Tm1 Tm2 Tm3 Tm4,Vf1 Vf2 Vf3 Vf4 0.113649845123291];
egpre=[Tm1 Tm2 Tm3 Tm4,Vf1 Vf2 Vf3 Vf4 0.113649845123291];
egpre2=[Tm1 Tm2 Tm3 Tm4,Vf1 Vf2 Vf3 Vf4 0.113649845123291];
...
% egpre=[Tm1 Tm2 Tm3 Tm4 Vf1 Vf2 Vf3 Vf4 1/9];
% eg=[Tm1 Tm2 Tm3 Tm4 Vf1 Vf2 Vf3 Vf4 1/9];
...
%     end
    ...
    [ee0gm,ee0gv,eg,tg,sg,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4,cnhist]=predictorg(xx,xpre1,xpre2,t,tpre1,tpre2,holdd,T,eg,egpre,tg,cp,cn,cq,avrov,gvrov,dae); %predictor of the intermediate points (explicit predictor)
    ...
    [iterations,xx,xxx,eg,tg,feeg,eeg,avrov,gvrov,fitert,dhiterations]=NewtonNL(h,xx0,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,tpre2,t,sg,eg,T,tg,ee0gm,ee0gv,egpre,cn,cq,dae,avrovpre,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovpre,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4,holdd,cnhist); %The main function (Newton solver)
...
figure (1) %Voltage bus 1 4 9
plot(times,sqrt(xhist(63,:).^2+xhist(64,:).^2),'r+')
hold on
%plot(times,sqrt(xhist(69,:).^2+xhist(70,:).^2),'+')
%plot(times,sqrt(xhist(79,:).^2+xhist(80,:).^2),'+')
xlabel('Time (s)','FontSize', 24)
ylabel('Voltage (Per unit)','FontSize', 24)
legend('V_{1} SRM','V_{1} SSM','V_{1} IBM','Location','southeast')
%legend('V_{1} SRM','V_{4} SRM','V_{9} SRM','V_{1} SSM','V_{4} SSM','V_{9} SSM','V_{1} IBM','V_{4} IBM','V_{9} IBM','Location','northeast')
set(gca,'FontSize',18)
xlim([0 tsim])
...
figure(2)
plot(times,xhist(29,:),'r+')
hold on
%plot(times,xhist(30,:),'+')
xlabel('Time','FontSize', 24)
ylabel('Frequency (Per unit)','FontSize', 24)
legend('f7 SRM','f7 SSM','f7 IBM','Location','northeast')
%legend('f7 SRM','f9 SRM','f7 SSM','f9 SSM','f7 IBM','f9 IBM','Location','northeast')
set(gca,'FontSize',18)
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'Kundurfrequency.pdf',figure_property);
end
...
% plot(times,xhist(22,:),'+')
% xlabel('Time','FontSize', 24)
% ylabel('POD input (Per unit)','FontSize', 24)
% legend('Without POD SRM','With POD SRM','Without POD IBM','With POD IBM','Location','northeast')
% set(gca,'FontSize',18)
% xlim([0 tsim])
% figure_property.units = 'inches';
% figure_property.format = 'pdf';
...
figure(3)
plot(times,xhist(75,:).*xhist(99,:)+xhist(76,:).*xhist(100,:),'r+')
hold on
xlabel('Time','FontSize', 24)
ylabel('Power (Per unit)','FontSize', 24)
set(gca,'FontSize',18)
xlim([0 tsim])
legend('P SRM','P SSM','P IBM','Location','northeast')
figure_property.units = 'inches';
figure_property.format = 'pdf';
figure_property.Preview= 'none';
figure_property.Width= '16'; % Figure width on canvas
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'KundurACPower.pdf',figure_property);
end

figure (30) %POD
plot(times,ehist(9,:),'r+')
hold on
xlabel('Time','FontSize', 24)
ylabel('POD output (Per unit)','FontSize', 24)
legend('SRM','SSM','IBM','Location','northeast')
set(gca,'FontSize',18)
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'KundurPOD.pdf',figure_property);
end
figure (31) %Governor
plot(times,ehist(1,:),'r+')
hold on
%plot(times,ehist(2,:),'+')
%plot(times,ehist(3,:),'+')
%plot(times,ehist(4,:),'+')
...
legend('G1 SRM','G1 SSM','G1 IBM','Location','northeast')
%legend('G1 SRM','G2 SRM','G3 SRM','G4 SRM','G1 SSM','G2 SSM','G3 SSM','G4 SSM','G1 IBM','G2 IBM','G3 IBM','G4 IBM','Location','northeast')
set(gca,'FontSize',18)
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
figure (32) %Exciter
plot(times,ehist(5,:).*2473.05660377359,'r+')
hold on
%plot(times,ehist(6,:).*3692.16901408451,'+')
%plot(times,ehist(7,:).*2621.44000000000,'+')
%plot(times,ehist(8,:).*3640.88888888889,'+')
xlabel('Time (s)','FontSize', 24)
...
function [iterations,xx,xxx,eg,tg,feeg,eegv,avrov,gvrov,fitert,dhiterations]=NewtonNL(h,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,tpre2,t,sg,eg,T,tg,eegm,eegv,egpre,cn,cq,dae,avrovpre,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovpre,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4,holdd,cnhist)
...
%         [JJ,fiter]=Jacob22(h,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,eg,egpre,t,dae,sg,eegm,cn,cnhist,holdd); %Simplified Jacobian with B (symbolic)
       ...
       %[JJ,fiter]=Jacob1(h,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,eg,egpre,t,dae,sg); %Simplified Jacobian
        fitert=fitert+fiter;
        [feeg,tg,cnhist,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4]=funcg(xx,xpre1,xpre2,eg,t,tpre1,tpre2,T,eegm,eegv,egpre,tg,h,holdd,cn,cq,avrovpre,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovpre,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4,dae); %Corrector of intermediate points (Implicit corrector)
...
gvrov(1,1)=gvrovmm1(:,end);
gvrov(1,2)=gvrovmm2(:,end);
gvrov(1,3)=gvrovmm3(:,end);
gvrov(1,4)=gvrovmm4(:,end);
end
...
function [eeeg,tg,cnhist,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4]=funcg(xx,xpre1,xpre2,eg,t,tpre1,tpre2,T,eegm,eegv,egpre,tg,h,holdd,cn,cq,avrovpre,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovpre,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4,dae)
tgpre=tg;
cni=0;
mm=0;
avra=0;
...
%             xg=(hg*(evaleval(xx,t,eg))+((1+hg/holdd)*xpre1)+((-(hg^2)/((hg+holdd)*holdd))*xpre2))/((2*hg+holdd)/(hg+holdd)); %Second order BDF implicit interpolator
            ...
            xg=quz(xg,cq);
            [f2,avrov,gvrov]=contim(eegm,egpre,T,xg,m,mm,cni,avrovpre,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovpre,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4); %This function contains the controllers' equations
            eeeg(1,mm)=f2;
            temp(1,cni)=tg(1,cni);
            ...
            elseif cni==1 || cni==2 || cni==3 || cni==4
                if cni==1
                    gvrovmm1(:,m)=gvrov;
                    gvra=gvra+1;
                elseif cni==2
...
function [f2,avrov,gvrov]=contim(eegm,egpre,T,xg,m,mm,cni,avrovpre,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovpre,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4)
if cni==1 || cni==2 || cni==3 || cni==4
            if m==1
                [aw,gvrov]=torque(egpre(1,cni),T(1,cni),xg,cni,gvrovpre(:,cni));
                aw=max(aw,-2); %upper limnit anti wind up
%                 aw=min(aw,1); %lower limnit anti wind up
                aw=min(aw,2); %lower limnit anti wind up
                f2=eegm(mm,cni)-aw;
                avrov=0;
            else
                if cni==1
                    gvrovtemp=gvrovmm1(:,m-1);
                elseif cni==2
                    gvrovtemp=gvrovmm2(:,m-1);
                elseif cni==3
                    gvrovtemp=gvrovmm3(:,m-1);
                elseif cni==4
                    gvrovtemp=gvrovmm4(:,m-1);
                end
                [aw,gvrov]=torque(eegm(mm-1,cni),T(1,cni),xg,cni,gvrovtemp);
                aw=max(aw,-2); %upper limnit anti wind up
%                 aw=min(aw,1); %lower limnit anti wind up
                aw=min(aw,2); %lower limnit anti wind up
                f2=eegm(mm,cni)-aw;
...
elseif cni==7
        global fuzzyAVR3;
        Vter3=sqrt(xg(67,1)^2+xg(68,1)^2);
        aw=evalfis(fuzzyAVR3,Vter3);
        f2=eegm(mm,cni)-aw;
        gvrov=0;
        avrov=0;
elseif cni==8
        global fuzzyAVR4;
        Vter4=sqrt(xg(69,1)^2+xg(70,1)^2);
        aw=evalfis(fuzzyAVR4,Vter4);
        f2=eegm(mm,cni)-aw;
        gvrov=0;
        avrov=0;
elseif cni==9
                if m==1
                aw=pod(egpre(1,cni),T(1,cni),xg);
                aw=max(aw,-1.5); %upper limnit anti wind up
% %                 aw=min(aw,1); %lower limnit anti wind up
                aw=min(aw,1.5); %lower limnit anti wind up
                f2=eegm(mm,cni)-aw;
                avrov=0;
                gvrov=0;
            else
                aw=pod(eegm(mm-1,cni),T(1,cni),xg);
                aw=max(aw,-1.5); %upper limnit anti wind up
% %                 aw=min(aw,1); %lower limnit anti wind up
                aw=min(aw,1.5); %lower limnit anti wind up
                f2=eegm(mm,cni)-aw;
...
function [ee0gm,ee0gv,eg,tg,sg,avrovmm1,avrovmm2,avrovmm3,avrovmm4,gvrovmm1,gvrovmm2,gvrovmm3,gvrovmm4,cnhist]=predictorg(xx,xpre1,xpre2,t,tpre1,tpre2,holdd,T,eg,egpre,tg,cp,cn,cq,avrov,gvrov,dae)
tgpre=tg;  
cni=0;
mm=0;
avra=0;
...
while cni<cn
    ...
    if tg(1,cni)>tpre1 && tg(1,cni)<=t+0.000001
        ...
        while (tg(1,cni)<=t+0.000001)
            ...
            elseif cni==1 || cni==2 || cni==3 || cni==4
                if cni==1
                    gvrovmm1(:,m)=gvrov(:,cni);
                    gvra=gvra+1;
                elseif cni==2
                    gvrovmm2(:,m)=gvrov(:,cni);
                    gvrb=gvrb+1;
                elseif cni==3
                    gvrovmm3(:,m)=gvrov(:,cni);
                    gvrc=gvrc+1;
                else
                    gvrovmm4(:,m)=gvrov(:,cni);
                    gvrd=gvrd+1;
                end
...
if cni==1 || cni==2 || cni==3 || cni==4
    [egtemp,gvrov(:,cni)]=torque(eg(1,cni),T(1,cni),xg,cni,gvrov(:,cni));
    egtemp=max(egtemp,-2); %upper limnit anti wind up
%     egtemp=min(egtemp,1); %lower limnit anti wind up
    egtemp=min(egtemp,2); %lower limnit anti wind up
elseif cni==5 || cni==6
...
elseif cni==7
    global fuzzyAVR3;
    Vter3=sqrt(xg(67,1)^2+xg(68,1)^2);
    egtemp=evalfis(fuzzyAVR3,Vter3);
elseif cni==8
    global fuzzyAVR4;
    Vter4=sqrt(xg(69,1)^2+xg(70,1)^2);
    egtemp=evalfis(fuzzyAVR4,Vter4);
elseif cni==9
    egtemp=pod(eg(1,cni),T(1,cni),xg);
    egtemp=max(egtemp,-1.5); %lower limnit anti wind up
    egtemp=min(egtemp,1.5); %upper limnit anti wind up
end
end

function [et2,gvrov]=torque(et2,T,xg,cni,gvrov)
%Data
kp=12;
ki=0.2;
Droop=1;
if cni==1
    ...
    Te=xg(37,1)*xg(36,1)-xg(38,1)*xg(35,1);
    wS=1+xg(3,1);
elseif cni==2
    zo=0.546951293945313;
    Te=xg(45,1)*xg(44,1)-xg(46,1)*xg(43,1);
    wS=1+xg(7,1);
elseif cni==3
    zo=0.433273315429688;
    Te=xg(53,1)*xg(52,1)-xg(54,1)*xg(51,1);
    wS=1+xg(11,1);
else
    zo=0.224349975585938;
    Te=xg(61,1)*xg(60,1)-xg(62,1)*xg(59,1);
    wS=1+xg(15,1);
end
...
if cni==5
    vo=1.03;
    Vter=sqrt(xg(63,1)^2+xg(64,1)^2);
    x62=xg(21,1);
elseif cni==6
    vo=1.01;
    Vter=sqrt(xg(65,1)^2+xg(66,1)^2);
    x62=xg(22,1);
elseif cni==7
    vo=1.03;
    Vter=sqrt(xg(67,1)^2+xg(68,1)^2);
    x62=xg(23,1);
else
    vo=1.01;
    Vter=sqrt(xg(69,1)^2+xg(70,1)^2);
    x62=xg(24,1);
end
kp=0.003; %0.01
ki=0.004; %0.01
