...
hmax=0.01; %The maximum acceptable time-step size
    ...
    if t<min(T)
        ...
        if mm==0
            ...
            while jjj<cn
                jjj=jjj+1;
                if mm(1,jjj)~=0
                    [et,avrov,gvrov]=contex(mm,et,T,cn,avrov,gvrov,xx,jjj); %Calculating the controllers' signal in case of any event
                end
            end
            et=quz(et,cq);
            flagg=1;
...
figure (1) %voltage
plot(times,sqrt((xhist(63,:)).^2+(xhist(64,:)).^2),'--','color',[0.4660 0.6740 0.1880])
hold on
...
figure(2)
plot(times,xhist(29,:),'--','color',[0.4660 0.6740 0.1880])
...
figure(3)
plot(times,xhist(75,:).*xhist(99,:)+xhist(76,:).*xhist(100,:),'--','color',[0.4660 0.6740 0.1880])
hold on
xlabel('Time','FontSize', 24)
...
figure (30) %POD
plot(times,ethist(9,:),'--','color',[0.4660 0.6740 0.1880])
hold on
xlabel('Time','FontSize', 24)
ylabel('POD signal','FontSize', 24)
set(gca,'FontSize',18)
figure (31) %Governor
plot(times,ethist(1,:),'--','color',[0.4660 0.6740 0.1880])
hold on
%plot(times,ethist(2,:),':')
%plot(times,ethist(3,:),':')
%plot(times,ethist(4,:),':')
...
set(gca,'FontSize',18)
figure (32) %Exciter
plot(times,ethist(5,:).*2473.05660377359,'--','color',[0.4660 0.6740 0.1880])
hold on
...
function [etj,avrov,gvrov]=contex(mm,etj,T,cn,avrov,gvrov,xx,jjj)
contj=jjj;
