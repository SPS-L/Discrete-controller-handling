%% %% This file solves the "Single Integral Controller" using the step reduction method (SRM)
...
tsim=50; %Time of the simulation
...
xpre1=xx;
xpre2=xx;
times(1)=t; %Allocating a space to save the times
...
etpre=0;
et=0; %Allocating a space to save the controller output
tg=T; %Time of the events
k=1;
ETOL=3e-4; %The tolerance to compare with the error estimate and set the time step size
...
%         et=max(et,-0.9); %lower limnit anti wind up
        et=quz(et,cq); %quantization of controller output
        t=tg;
    ...
    iterationsc=iterations+iterationsc; %Updating the number of Newton iterations
