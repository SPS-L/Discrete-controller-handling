...
save=0; %set to 1 to save the figures as PDF
tic
while t<tsim
    %Setting old times
    if k>2
    ...
    [iterations,xx,eg,tg,eeg]=NewtonNL(h,holdd,xx0,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,t,sg,eg,T,tg,G,u,ee0g,egpre,cq); %The main function (Newton solver)
...
if save==1
    hgexport(gcf,'SysConSinglePI.pdf',figure_property);
end
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'SysconSinglePIstep.pdf',figure_property);
end
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'SysconSinglePIerror.pdf',figure_property);
end
...
set(gca,'FontSize',18)
xlim([0 tsim])
legend('(IBM)','(DIBM)','Location','northwest')
set(gca, 'FontName', 'Times New Roman')

function [iterations,xx,eg,tg,eeg]=NewtonNL(h,holdd,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,t,sg,eg,T,tg,G,u,eeg,egpre,cq)
