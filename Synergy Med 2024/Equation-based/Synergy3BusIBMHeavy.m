...
set(gca,'FontSize',18)
legend('SRM','IBM','LIBM','Location','northeast')
set(gca, 'FontName', 'Times New Roman')
xlim([0 tsim])
figure_property.units = 'inches';
figure_property.format = 'pdf';
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'SyNERGYavr.pdf',figure_property);
end
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'SyNERGYstepsize.pdf',figure_property);
end
...
a=1;
fitert=0;
[feeg,tg,cnhist,avrovmm,gvrovmm,nm]=funcg(xx,xpre1,xpre2,eg,t,tpre1,tpre2,T,eegm,eegv,egpre,tg,h,holdd,cn,cq,avrovpre,avrovmm,gvrovpre,gvrovmm,dae); %Corrector of intermediate points (Implicit corrector)
    while (iterations<10) && (a>NTOL)
        ...
        if sg==0
            ...
            if nm(1,1)~=0 && nm(1,2)~=0
                xxx=[xx;eg'];
                sg2=2;
            elseif nm(1,1)==0 && nm(1,2)==0
                xxx=xx;
                sg2=0;
            elseif nm(1,1)==0 && nm(1,2)~=0
                xxx=[xx;eg(1,2)];
...
%       [JJ,fiter]=Jacob3(h,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,eg,egpre,t,dae,sg,eegm,eegv,cn,avrovpre,avrovmm,tg,tpre2,T,holdd,cq);
        %B
%         [JJ,fiter]=Jacob2(h,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,eg,egpre,t,dae,sg,eegm,cn); %Simplified Jacobian with B
       [JJ,fiter]=Jacob1(h,xx,xpre1,xpre2,alpha1,alpha2,alpha0,betha1,betha2,tpre1,eg,egpre,t,dae,sg2); %Simplified Jacobian
%         fitert=fitert+fiter;
        ...
        if sg==0
            ...
            if nm(1,1)~=0 && nm(1,2)~=0
                f=[fy;feeg(1,nm(1,1));feeg(1,nm(1,2))];
            elseif nm(1,1)==0 && nm(1,2)==0
                f=fy;
            elseif nm(1,1)==0 && nm(1,2)~=0
                f=[fy;feeg(1,nm(1,2))];
        ...
        if sg~=0
            if nm(1,1)~=0 && nm(1,2)~=0
                egtemp=xxx(s+1:end,1);
                egtemp=quz(egtemp,cq);
                eg(1,1)=egtemp(1,1);
                eg(1,2)=egtemp(2,1);
            elseif nm(1,1)==0 && nm(1,2)==0
                eg=quz(egtemp,cq);
            elseif nm(1,1)==0 && nm(1,2)~=0
                egtemp(2,1)=xxx(s+1:end,1);
                egtemp=quz(egtemp,cq);
                eg(1,2)=egtemp(2,1);
            elseif nm(1,1)~=0 && nm(1,2)==0
                egtemp(1,1)=xxx(s+1:end,1);
                egtemp=quz(egtemp,cq);
                eg(1,1)=egtemp(1,1);
            end
%             
...
cni=0;
    while cni<cn
        cni=cni+1;
        tg(1,cni)=tg(1,cni)+(nm(1,cni))*T(1,cni);
    end
else
    tg=tg;
end
...
function [eeeg,tg,cnhist,avrovmm,gvrovmm,nm]=funcg(xx,xpre1,xpre2,eg,t,tpre1,tpre2,T,eegm,eegv,egpre,tg,h,holdd,cn,cq,avrovpre,avrovmm,gvrovpre,gvrovmm,dae)
tgpre=tg;
cni=0;
mm=0;
avrc=0;
...
%             xg=(hg*(evaleval(xx,t,eg))+((1+hg/holdd)*xpre1)+((-(hg^2)/((hg+holdd)*holdd))*xpre2))/((2*hg+holdd)/(hg+holdd)); %Second order BDF implicit interpolator
            ...
            xg=quz(xg,cq);
            [f2,avrov,gvrov]=contim(eegm,egpre,T,xg,m,mm,cni,avrovpre,avrovmm,gvrovpre,gvrovmm); %This function contains the controllers' equations
            eeeg(1,mm)=f2;
            temp(1,cni)=tg(1,cni);
...
function [f2,avrov,gvrov]=contim(eegm,egpre,T,xg,m,mm,cni,avrovpre,avrovmm,gvrovpre,gvrov)
if cni==1
            if m==1
