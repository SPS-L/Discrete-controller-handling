...
if save==1
    hgexport(gcf,'SinglePI.pdf',figure_property);
end
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'SinglePIstep.pdf',figure_property);
end
...
set(chosen_figure,'Units','inches');
if save==1
    hgexport(gcf,'singlepierror.pdf',figure_property);
end
...
%%%chossing the interpolator (only the chosen one should be decomented)
         xg=xpre1+hg*(evaleval(xx,t,eg)); %First order AM implicit interpolator
